...
def is_passphrase(string):
    words = string.split()
    passphrase = []
    for word in words:
        if word not in passphrase:
            passphrase.append(word)
    if len(passphrase) == len(words):
        return True
    return False


def count_passphrases(array_string):
    total = 0
    for string in array_string:
        if is_passphrase(string):
            total += 1
    return total
...
def is_passphrase_v2(string):
    words = string.split()
    passphrase = []
    for word in words:
        sorted_word = ''.join(sorted(word))
        if sorted_word not in passphrase:
            passphrase.append(sorted_word)
    if len(passphrase) == len(words):
        return True
    return False


def count_passphrases_v2(array_string):
    total = 0
    for string in array_string:
        if is_passphrase_v2(string):
            total += 1
    return total
